...
아래 이미지는 이 브랜치의 최신 CSS를 적용한 **1280px 데스크톱 뷰포트**에서 다시 캡처했다. 헤더는 화면 전체 너비를 사용하고, 본문 카드는 최대 1200px로 중앙 정렬된다. 표가 넓어지는 경우에는 카드 안에서만 가로 스크롤된다.
...
진료기록 상세에서 AI 예측 실행 후 결과 목록에 예측 시각·결과·신뢰도·모델명이 표시되는 것을 확인했다.
...
5. 좁은 화면에서 메뉴·버튼 글자가 글자 단위로 줄바꿈되거나 표 열이 과도하게 좁아지는 문제를 보완했다. 메뉴·버튼은 단어 단위로 유지하고, 검색 필터는 세로로 재배치하며, 표는 카드 안에서 가로 스크롤된다. 스타일시트 URL에 버전을 붙여 배포 뒤에도 이전 CSS 캐시가 남지 않게 했다.
...
- 1280px 폭 브라우저 확인: 헤더 1280px, 본문 1200px, 환자 목록 카드 1160px이며 문서 전체의 가로 넘침 없음
