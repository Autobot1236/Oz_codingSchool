...
| 구분 | 기본 경로 |
| --- | --- |
| 환자 관리 API | `/api/patients` |
| 진료기록 API | `/api/medical-records` |
...
| `gender` | string | Y | 환자 성별 | `male`, `female` 중 하나 |
...
```http
GET /api/patients?name=김&gender=male&min_age=20&max_age=80&page=1&size=20
Authorization: Bearer {access_token}
```
...
| 요구사항 ID | REQ-MDR-002 |
| Method | `GET` |
| URL | `/api/patients/{patient_id}/medical-records` |
| 인증 | 필요 |
| 권한 | `STAFF`, `ADMIN` |
...
```http
GET /api/patients/1/medical-records?page=1&size=20
Authorization: Bearer {access_token}
```
